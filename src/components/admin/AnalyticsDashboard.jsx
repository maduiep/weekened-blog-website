  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        {metrics.map((m, i) => (
          <div key={i} style={{ background: 'white', padding: '24px', borderRadius: '12px', border: '1px solid var(--color-border)', boxShadow: '0 4px 6px rgba(0,0,0,0.02)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '16px', color: 'var(--color-text-muted)' }}>
              {m.icon}
              <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--color-sport-green)' }}>{m.trend}</span>
            </div>
            <div style={{ fontSize: '28px', fontWeight: 800, color: 'var(--color-dark)', marginBottom: '4px' }}>{m.value}</div>
            <div style={{ fontSize: '13px', color: 'var(--color-text-secondary)' }}>{m.label}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '24px' }}>
        ...
        {/* Most Read Articles */}
        <div style={{ background: 'white', padding: '24px', borderRadius: '12px', border: '1px solid var(--color-border)' }}>
          <h3 style={{ fontSize: '16px', marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '8px' }}><FileText size={18} /> Top Content</h3>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: i !== 4 ? '1px solid #eee' : 'none', paddingBottom: i !== 4 ? '12px' : 0 }}>
                <span style={{ fontSize: '13px', fontWeight: 600, color: 'var(--color-dark)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '200px' }}>{article.title}</span>
                <span style={{ fontSize: '12px', color: 'var(--color-primary)', fontWeight: 700 }}>{article.views}</span>
              </div>
            ))}
          </div>
        </div>
